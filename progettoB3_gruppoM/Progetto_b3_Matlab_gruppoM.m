...
RR_a = (1 + tau*s)/(1 + alpha_tau*s) % rete anticipatrice
...
RR_d = mu_d*RR_a
RR = RR_s*RR_d
LL = RR_d*GG_e % funzione di anello
...
figure(8)
hold on, grid on, zoom on
title('Risposta complessiva del sistema')
plot(tt,ww,'m')
plot(tt,y_tot,'b')
grid on
legend('w(t)','y_{tot}(t)')
...
figure(9)
hold on, grid on, zoom on
title('Errore complessivo del sistema')
plot(tt,e_tot,'b')
grid on
legend('e(t)')
